...
const SkillTree = ({ isGodMode }) => {
  ...
  return (
    <div className={`max-w-5xl mx-auto border-4 p-8 transition-all duration-1000 fade-in ${
      isGodMode 
      ? 'bg-black/90 border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.3)]' 
      ...
      <h2 className={`text-xl mb-10 border-b-4 border-dashed pb-4 uppercase transition-colors ${
        isGodMode ? 'text-yellow-300 border-yellow-900' : 'text-yellow-400 border-gray-700'
      }`}>
        ...
        {tree.map((node, i) => (
          <div 
            ...
            className={`border-4 p-5 transition-all group ${
              isGodMode 
              ? 'border-yellow-900 bg-[#1a1400]/50 hover:border-yellow-400 hover:bg-yellow-950/40' 
          ...
          >
            ...
            <ul className="space-y-4">
              {node.skills.map((skill, j) => (
                <li key={j} className="text-white text-[9px] flex items-center gap-2 group/item">
                  ...
                  <span className={isGodMode ? 'text-yellow-100/90' : 'text-white'}>
                    {skill}
                  </span>
                </li>
              ))}
            ...
            {isGodMode && (
              <div className="mt-6 pt-4 border-t border-yellow-900/50 text-right">
                <span className="text-[7px] text-yellow-600 animate-pulse font-bold tracking-tighter">MAX_LEVEL</span>
              </div>
            )}
          </div>
        ))}
